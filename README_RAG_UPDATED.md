...
## 11A. Enterprise RAG AI Upgrade (Version 1.2)

![Enterprise RAG Architecture](docs/images/architecture-slide-13.png)

### Why This Upgrade Was Added

Modern enterprise AI solutions require continuously changing business knowledge such as return policies, refund policies, delivery policies, cancellation rules, and business processes.

Instead of retraining an LLM whenever business policies change, Amitra Commerce Mesh was upgraded with a Retrieval Augmented Generation (RAG) architecture.

### Enterprise RAG Knowledge Loading Pipeline

```text
Policy PDF Upload (Google Cloud Storage)
        ↓
Eventarc Trigger
        ↓
Google Cloud Function (Serverless)
        ↓
Read Policy PDF + MongoDB Product Data
        ↓
...
- Executes only when policy documents change
- No dedicated ingestion service running 24x7
- Pay-per-use model
- Event-driven architecture
- Enterprise scalability

### Why Self-Hosted Embedding Model

- Lower cost than external embedding APIs
- Lower latency
- Enterprise data control
- Policies remain within the enterprise boundary
...
- Enterprise knowledge ownership
- Scalable cloud-native AI architecture
